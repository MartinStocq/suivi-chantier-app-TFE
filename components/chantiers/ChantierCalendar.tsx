                ...
                <>
                  ...
                  <div className="space-y-0.5">
                    {items.slice(0, 2).map(c => {
                      const style = S[c.statut] ?? S.ENATTENTE
                      return (
                        <Link
                          key={c.id}
                          href={`/chantiers/${c.id}`}
                          title={`${c.titre}${c.client ? ' — ' + c.client.nom : ''}`}
                          className={[
                            'flex items-center gap-1 px-1.5 py-0.5 rounded border',
                            'text-xs truncate hover:opacity-70 transition-opacity',
                            style.pill,
                          ].join(' ')}
                        >
                    ...
                    {items.length > 2 && (
                      <p className="text-xs text-gray-400 px-1">+{items.length - 2} autres</p>
                    )}
                  </div>
                </>
              )}
