        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white border border-gray-200 rounded-xl">
            <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
              <h2 className="text-sm font-semibold text-gray-900">Chantiers récents</h2>
              <Link href="/chantiers" className="text-xs text-gray-400 hover:text-gray-700 transition">
                Voir tout
              </Link>
            </div>
            <div className="divide-y divide-gray-50">
              {recentChantiers.length === 0 ? (
                <div className="px-5 py-10 text-center">
                  <p className="text-sm text-gray-400">Aucun chantier créé</p>
                </div>
                ...
                <Link key={c.id} href={`/chantiers/${c.id}`}
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{c.client.nom} · {c.adresse.ville}</p>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-md border font-medium ${
                    ...
                    {c.statut === 'EN_COURS' ? 'En cours' : c.statut === 'EN_ATTENTE' ? 'En attente' : c.statut === 'TERMINE' ? 'Terminé' : 'Suspendu'}
                  </span>
                </Link>
              ))}
            </div>
